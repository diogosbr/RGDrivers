...
#'
#' Esta função insere novas colunas (ex: `link_mag`, `D_LINK`) em um objeto `SpatVector`
#' a partir de um `data.frame` atualizado e o grava em disco.
#'
#' @param vetor_original Objeto `SpatVector` original (rede hidrográfica)
#' @param df_atualizado Data frame contendo os IDs dos trechos e as colunas novas
#' @param col_id Nome da coluna de identificação única dos trechos (ex: "cotrecho")
#' @param colunas_adicionais Vetor com os nomes das colunas que serão adicionadas ao vetor
#' @param caminho_saida Caminho completo para salvar o arquivo (ex: "saida.gpkg")
#' @param overwrite Lógico, se TRUE permite sobrescrever o arquivo existente
#'
